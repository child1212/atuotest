...
def hand_loop(template,jldw,hand_set_gray,ans_tuple_gray):
    #点击酒令，寻找酒令一
    print("点击酒令，寻找酒令")
    ...
    while running<3:
        ...
        for i in range(len(hand_set_gray)):
            if icon_exist_quick(hand_set_gray[i],template):
                if i < 5:
                    ans_l.append(i+1)
                else:
                    ans_l.append(i-4)
        ...
        result = mouse_click(box,find_icon_quick(ans_path,template,0.99))
        if result == False:
            running += 1
...
def whole_loop(x=1,y="y",i=1,nouse=False):
    ...
    while(a<x):
        hand_loop(template,i,hand_set_gray,ans_tuple_gray)
        if y == "n":
