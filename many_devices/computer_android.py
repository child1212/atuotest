...
import scrcpy
from scrcpy_ui.ui_main import Ui_MainWindow
...
class MainWindow(QMainWindow):
    ...
    def on_mouse_event(self, action=scrcpy.ACTION_DOWN):
        def handler(evt: QMouseEvent):
            ...
            act=evt.position().x() / ratio, evt.position().y() / ratio, action
            if act[2] == 0:
                    self.record.append([act[0],act[1]])
            elif act[2] == 1:
                    self.record.append([act[0],act[1]])
            ...
            if len(self.record) == 2:
                if self.record[0] == self.record[1]:
                    for de in self.devices:
                        if de != self.device.serial:
                            a = adb.device(de)
                            a.shell("input tap {x} {y}".format(x=self.record[0][0],y=self.record[0][1]))
                else:
                    for de in self.devices:
                        if de != self.device.serial:
                            a = adb.device(de)
                            a.shell("input swipe {x1} {y1} {x2} {y2}".format(x1=self.record[0][0],y1=self.record[0][1],x2=self.record[1][0],y2 = self.record[1][1]))
                self.record = []
        return handler
