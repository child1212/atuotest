...
class DeviceAndroid():
    ...
    def swipe(self,position_x,position_y,position_x1,position_y1,direction):
        if direction == 0:
            print("swipe",position_x,position_y,position_x1,position_y1,0.2)
            self.adb_d.swipe(position_x,position_y,position_x1,position_y1,0.2)
        else:
            print("swipe",position_y,1-position_x,position_y1,1-position_x1,0.2)
                    ...
                    if search_result[1] == "UP":
                        if len(position) == 1:
                            for shouji in devices_set:
                                threading.Thread(target=shouji.tap,args=(position[0][0],position[0][1],MainDevice.check_direction())).start()
                        elif len(position) == 2:
                            if abs(position[0][0]-position[1][0])>distance or abs(position[0][1]-position[1][1])>distance:
                                for shouji in devices_set:
                                    threading.Thread(target=shouji.swipe,args=(position[0][0],position[0][1],position[1][0],position[1][1],MainDevice.check_direction())).start()
                            else:
                                for shouji in devices_set:
                                    threading.Thread(target=shouji.tap,args=(position[0][0],position[0][1],MainDevice.check_direction())).start()
                        position=[]
