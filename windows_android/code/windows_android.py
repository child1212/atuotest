...
def adb_click(position):
    os.system("adb -e input tap {x} {y}".format(x=position[0],y=position[1]))    
...
def get_zx_screen(window_name):
    '''
    window_name:截取窗口的名称,要求窗口在主屏幕最上方
    '''
    name = window_name
    window = FindWindow(0,name)
    # shell = win32com.client.Dispatch("WScript.Shell")
...
def hand_loop(jldw,hand_set_gray,ans_tuple_gray,window_name):
    #点击酒令，寻找酒令一
    time.sleep(1)
    print("点击酒令，寻找酒令")
    ...
    if jiuling[0] == False:
        mouse_click(box,find_icon(jiuling_title_path,image))
    #点击酒令一
    print("点击酒令一子菜单")
    box,image = get_zx_screen(window_name)
...
def whole_loop(window_name,x,y,i,nouse=False):
    ...
    while(a<x):
        hand_loop(i,hand_set_gray,ans_tuple_gray,window_name)
        if y == "n":
